...
void setup() {
  ...
  while (soap.getServerInfo(i, &serv)) {
    ...
    // send search request to media server: title must contain the string "hard" & genre must contain the string "pop"
    if (!soap.searchServer(i, "0", &searchResult, SOAP_SEARCH_CRITERIA_GENRE, "pop",
                                                  SOAP_SEARCH_CRITERIA_TITLE, "hard")) {
